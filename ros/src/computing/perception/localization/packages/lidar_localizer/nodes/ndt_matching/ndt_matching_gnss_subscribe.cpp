...
#include <autoware_msgs/NdtPoseAndRTKPose.h>
...
typedef message_filters::sync_policies::ApproximateTime<geometry_msgs::PoseStamped, geometry_msgs::PoseStamped, autoware_msgs::GnssStandardDeviation> NdtRtkSync;
static geometry_msgs::PoseStamped RTK_pose_msg, RTK_localizer_pose_msg;
static autoware_msgs::GnssStandardDeviation RTK_standard_deviation_msg;
static ros::Publisher NdtPoseAndRTKPose_pub;
...
static void RTK_gnss_callback(const geometry_msgs::PoseStampedConstPtr& rtk_base_link_pose, const geometry_msgs::PoseStampedConstPtr& rtk_localizer_pose,
                              const autoware_msgs::GnssStandardDeviationConstPtr& deviation)
{
  ...
  RTK_standard_deviation_msg.header.seq = deviation->header.seq;
  RTK_standard_deviation_msg.header.stamp = deviation->header.stamp;
  RTK_standard_deviation_msg.header.frame_id = deviation->header.frame_id;
  RTK_standard_deviation_msg.lat_std = deviation->lat_std;
  RTK_standard_deviation_msg.lon_std = deviation->lon_std;
  RTK_standard_deviation_msg.alt_std = deviation->alt_std;
}
...
static void points_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  ...
  if (map_loaded == 1 && init_pos_set == 1)
  {
    ...
    ndtpose_and_rtkpose.RTK_standard_deviation.header.frame_id = "/map";
    ndtpose_and_rtkpose.RTK_standard_deviation.header.stamp = current_scan_time;
...
int main(int argc, char** argv)
{
  ...
  NdtPoseAndRTKPose_pub = nh.advertise<autoware_msgs::NdtPoseAndRTKPose>("/ndt_pose_and_RTK_pose", 10);
  ...
  message_filters::Subscriber<autoware_msgs::GnssStandardDeviation> rtk_deviation_sub(nh, "/gnss_standard_deviation", 10);
  message_filters::Synchronizer<NdtRtkSync> sync(NdtRtkSync(10), rtk_sub, rtk_localizer_sub, rtk_deviation_sub);
  sync.registerCallback(boost::bind(&RTK_gnss_callback, _1, _2, _3));
