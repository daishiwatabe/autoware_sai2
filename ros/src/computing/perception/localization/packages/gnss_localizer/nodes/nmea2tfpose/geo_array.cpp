...
geo_array::geo_array(unsigned int arraySize)
{
    ...
    hdt_list=new double[listSize];

    for(unsigned int i=0;i<listSize;i++)
    {
        yaw_list[i]=0; lat_std[i]=0; lon_std[0]=0; alt_std[0]=0; surface_speed[i]=0; hdt_list[i]=0;
    }
}
...
void geo_array::push_back(const geo_pos_conv &gpc, double latstd, double lonstd, double altstd,
...
double geo_array::returnHDT_movingAverage()
{
    ...
    for(unsigned int i=0; i<listSize; i++)
    {
        x += cos(hdt_list[i]);  y += sin(hdt_list[i]);
    }
    x /= listSize;  y /= listSize;
    return atan2(x,y);
}
